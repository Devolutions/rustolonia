...
public class ClrTypeExtractorTests
{
    ...
    public void Projects_kernel_types_properties_commands_and_nearest_bases()
    {
        ...
        Assert.Equal(12, controlEvents.Count);
        ...
        // Wave P: KeyUp mirrors KeyDown; GotFocus carries the navigation context; LostFocus
        // is a plain notification.
        var keyUp = controlEvents.Single(@event => @event.Name == "KeyUp");
        Assert.Equal(EventPayloadKind.Fields, keyUp.PayloadKind);
        Assert.Equal(5, keyUp.Parameters.Count);
        Assert.Equal(
            ParameterDirection.InOut,
            keyUp.Parameters.Single(parameter => parameter.Name == "Handled").Direction);
        var gotFocus = controlEvents.Single(@event => @event.Name == "GotFocus");
        Assert.Equal(EventPayloadKind.Fields, gotFocus.PayloadKind);
        Assert.Equal(2, gotFocus.Parameters.Count);
        Assert.Equal(
            EventPayloadKind.None,
            controlEvents.Single(@event => @event.Name == "LostFocus").PayloadKind);
        // Wave Q: the pointer events carry the device-independent payload that does
        // not need a host-typed object (modifiers; the wheel adds its Vector delta).
        // DoubleTapped has no payload of its own.
        foreach (var eventName in new[] { "PointerEntered", "PointerExited", "Tapped" })
        {
            var pointerEvent = controlEvents.Single(@event => @event.Name == eventName);
            Assert.Equal(EventPayloadKind.Fields, pointerEvent.PayloadKind);
            Assert.Equal("KeyModifiers", Assert.Single(pointerEvent.Parameters).Name);
        }
        var wheel = controlEvents.Single(@event => @event.Name == "PointerWheelChanged");
        Assert.Equal(EventPayloadKind.Fields, wheel.PayloadKind);
        Assert.Equal(MarshallingKind.Vector,
            wheel.Parameters.Single(parameter => parameter.Name == "Delta").Kind);
        Assert.Equal("KeyModifiers", wheel.Parameters[1].Name);
        Assert.Equal(
            EventPayloadKind.None,
            controlEvents.Single(@event => @event.Name == "DoubleTapped").PayloadKind);
        Assert.Equal(12, controlEvents.Count);
        Assert.All(
    ...
    public void Layout_members_keep_the_abi_version_of_the_interfaces_they_widened()
    {
        ...
        Assert.Equal(10, Type(ir, "IAvnControl").AbiVersion);
        Assert.Equal(12, Type(ir, "IAvnDecorator").AbiVersion);
        Assert.Equal(
            ClrTypeExtractor.CreateDeterministicIid(
                Type(ir, "IAvnDecorator").FullName, 12),
    ...
    public void Chrome_members_keep_the_abi_version_of_the_interfaces_they_widened()
    {
        ...
        Assert.All(
            ...
            name => { var type = Type(ir, name); Assert.Equal(13, type.AbiVersion);
                Assert.Equal(
        ...
        Assert.All(
            ...
            name =>
            {
                ...
                Assert.Equal(14, type.AbiVersion);
                Assert.Equal(
                    ClrTypeExtractor.CreateDeterministicIid(type.FullName, 14),
                    type.Iid);
            });
    ...
    public void Completeness_members_keep_the_abi_version_of_every_widened_interface()
    {
        ...
        Assert.Equal(14, Type(ir, "IAvnGrid").AbiVersion);
        Assert.Equal(
            ClrTypeExtractor.CreateDeterministicIid(Type(ir, "IAvnGrid").FullName, 14),
        ...
        Assert.All(
            ...
            {
                var type = Type(ir, name);
                Assert.Equal(17, type.AbiVersion);
                Assert.Equal(
                        ClrTypeExtractor.CreateDeterministicIid(type.FullName, 17),
                        type.Iid);
            });

        var window = Type(ir, "IAvnWindow");
        Assert.Equal(20, window.AbiVersion);
        Assert.Equal(
            ClrTypeExtractor.CreateDeterministicIid(window.FullName, 20),
            window.Iid);
    }
    ...
    public void Wave_a_controls_publish_new_interfaces_at_version_one()
    {
        ...
        Assert.Equal(10, Type(ir, "IAvnImage").AbiVersion);
        // U17 republished TabItem with the items lineage; ToolTip was untouched.
        Assert.Equal(13, Type(ir, "IAvnTabItem").AbiVersion);
        Assert.Equal(11, Type(ir, "IAvnToolTip").AbiVersion);
        Assert.All(
            new[]
            {
            ...
            name => { var type = Type(ir, name); Assert.Equal(14, type.AbiVersion);
            });
        ...
        var pinned = new Dictionary<string, int>(StringComparer.Ordinal)
        {
            ...
            ["IAvnDecorator"] = 12,
            ["IAvnItemsControl"] = 17,
            ["IAvnSelectingItemsControl"] = 17,
            ["IAvnContentControl"] = 15,
            ["IAvnHeaderedContentControl"] = 15,
        };
    ...
    public void Projects_the_wave_a_controls_onto_the_types_that_declare_them()
    {
        ...
        // Wave P: it carries the added/removed items on a host-implemented args interface.
        ...
        Assert.Equal(EventPayloadKind.Args, treeSelectionChanged.PayloadKind);
        Assert.Equal(
            ["AddedItems", "RemovedItems"],
            treeSelectionChanged.Parameters.Select(parameter => parameter.Name));
    ...
    public void Wave_b_controls_publish_new_interfaces_at_version_one()
    {
        ...
        Assert.All(
            new[] { "IAvnMenuBase", "IAvnMenu", "IAvnHeaderedSelectingItemsControl" }, name => Assert.Equal(14, Type(ir, name).AbiVersion));
        Assert.Equal(17, Type(ir, "IAvnMenuItem").AbiVersion);
    ...
    public void Wave_d_button_family_publishes_new_interfaces_at_version_one()
    {
        ...
        Assert.Equal(17, Type(ir, "IAvnContextMenu").AbiVersion);
        Assert.All(
            new[]
            {
    ...
    public void Wave_e_input_controls_publish_new_interfaces_at_version_one()
    {
        ...
        Assert.Equal(11, spinner.AbiVersion);
        ...
        Assert.Equal(13, Type(ir, "IAvnSelectableTextBlock").AbiVersion);
        Assert.True(Type(ir, "IAvnSelectableTextBlock").IsConstructible);
        Assert.Equal(12, Type(ir, "IAvnNumericUpDown").AbiVersion);
        Assert.Equal(18, Type(ir, "IAvnAutoCompleteBox").AbiVersion);
    ...
    public void Wave_f_calendar_family_publishes_new_interfaces_at_version_one()
    {
        ...
        Assert.Equal(14, Type(ir, "IAvnCalendar").AbiVersion);
        Assert.Equal(13, Type(ir, "IAvnCalendarDatePicker").AbiVersion);
    ...
    public void Wave_g_content_chrome_publishes_new_interfaces_at_version_one()
    {
        ...
        Assert.Equal(12, Type(ir, "IAvnLabel").AbiVersion);
    ...
    public void Wave_h_shapes_publish_new_interfaces_at_version_one()
    {
        ...
        Assert.Equal(10, shape.AbiVersion);
    ...
    public void Wave_l_window_chrome_bumps_only_window()
    {
        ...
        Assert.Equal(15, Type(ir, "IAvnContentControl").AbiVersion);
    }
    ...
    public void Wave_m_fonts_bump_templated_control_and_text_block()
    {
        ...
        var templated = Type(ir, "IAvnTemplatedControl");
        Assert.Equal(14, templated.AbiVersion);
        Assert.All(
        ...
        var textBlock = Type(ir, "IAvnTextBlock");
        Assert.Equal(16, textBlock.AbiVersion);
        Assert.All(
        ...
        Assert.Equal(13, Type(ir, "IAvnSelectableTextBlock").AbiVersion);
        Assert.Equal(14, Type(ir, "IAvnBorder").AbiVersion);
    ...
    public void Wave_n_textbox_remainder_bumps_only_textbox_and_masked_textbox()
    {
        ...
        Assert.Equal(20, textBox.AbiVersion);
        Assert.All(
    ...
    public void Wave_o_combo_and_scroll_use_vector_and_size()
    {
        ...
        Assert.Equal(19, combo.AbiVersion);
    ...
    public void Wave_p_projects_instance_flyout_and_menu_item_open()
    {
        var ir = ClrTypeExtractor.Extract(KernelTypes, AvaloniaProjectionProfiles.ObjectModelKernel);
        var button = Type(ir, "IAvnButton");
        Assert.Equal(18, button.AbiVersion);
    ...
    public void Items_control_leftovers_keep_items_and_selection()
    {
        ...
        Assert.Equal(15, Type(ir, "IAvnContentControl").AbiVersion);
        Assert.Equal(20, Type(ir, "IAvnWindow").AbiVersion);
        Assert.Equal(13, ir.FactoryAbiVersion);
    }
    ...
    public void Text_leftovers_project_line_metrics_and_scroll_to_line()
    {
        ...
        Assert.Equal(13, selectable.AbiVersion);
    ...
    {
        var ir = ClrTypeExtractor.Extract(KernelTypes, AvaloniaProjectionProfiles.ObjectModelKernel);
        Assert.Equal(14, Type(ir, "IAvnCommandBar").AbiVersion);
        ...
        Assert.Equal(15, Type(ir, "IAvnCarousel").AbiVersion);
        Assert.Contains(Type(ir, "IAvnCarousel").Methods, m => m.Name == "Next");
        Assert.Equal(19, Type(ir, "IAvnComboBox").AbiVersion);
        Assert.Equal(13, Type(ir, "IAvnDatePicker").AbiVersion);
        Assert.Contains(Type(ir, "IAvnDatePicker").Properties, p => p.Name == "VerticalContentAlignment");
        Assert.Equal(17, Type(ir, "IAvnContextMenu").AbiVersion);
        Assert.Equal(15, Type(ir, "IAvnProgressBar").AbiVersion);
        Assert.Contains(Type(ir, "IAvnProgressBar").Properties, p => p.Name == "Percentage");
        Assert.Equal(14, Type(ir, "IAvnStackPanel").AbiVersion);
        Assert.Equal(14, Type(ir, "IAvnBorder").AbiVersion);
    ...
    public void Overlay_chrome_projects_popup_open_close_and_placement_target()
    {
        ...
        Assert.Equal(10, popup.AbiVersion);
    ...
    public void Control_leftovers_project_context_menu_and_loaded()
    {
        var ir = ClrTypeExtractor.Extract(KernelTypes, AvaloniaProjectionProfiles.ObjectModelKernel);
        var control = Type(ir, "IAvnControl");
        Assert.Equal(10, control.AbiVersion);
        ...
        Assert.Equal(6, Type(ir, "IAvnStyledElement").AbiVersion);
        Assert.Equal(18, Type(ir, "IAvnButton").AbiVersion);
        Assert.Equal(4, Type(ir, "IAvnFlyout").AbiVersion);
        Assert.Equal(13, ir.FactoryAbiVersion);
    }
